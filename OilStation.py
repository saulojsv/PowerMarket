...
from newspaper import Article # Importante: adicione ao requirements.txt
...
class XTINeuralEngine:
    ...
    def scrap_full_article(self, url):
        """Abre e extrai o texto real da notícia"""
        try:
            article = Article(url)
            article.download()
            article.parse()
            return article.text[:4000] # Limite para não estourar tokens
        except:
            return None

    def get_deep_neural_analysis(self, content, is_url=False):
        """Analisa o contexto completo do texto escaneado"""
        if not self.model: return 0.0, "AI INACTIVE"
        try:
            role = "Analista Senior de Petróleo"
            target = "Impacto no WTI Crude Oil"
    ...
    def process_input(self, text):
        """Decide se é uma URL (Scrapping) ou Título (Léxico)"""
...
def main():
    ...
    st.markdown(f"### < XTI/USD NEURAL TERMINAL v10.1 // DEEP READER >")
    ...
    with tab_ops:
        ...
        with col_verdict:
            ...
            fig = go.Figure(go.Scatter(y=prices[-50:], line=dict(color='#00FF41', width=3), fill='tozeroy'))
