...
def get_market_intelligence():
    try:
        # Usando intervalo de 5m para maior estabilidade
        xti = yf.download("CL=F", period="2d", interval="5m", progress=False)
        dxy = yf.download("DX-Y.NYB", period="2d", interval="5m", progress=False)
        
        if xti.empty or dxy.empty:
            return None, 0.0
            
        prices = xti['Close'].iloc[:, 0].dropna().tolist() if isinstance(xti['Close'], pd.DataFrame) else xti['Close'].dropna().tolist()
        dxy_close = dxy['Close'].iloc[:, 0] if isinstance(dxy['Close'], pd.DataFrame) else dxy['Close']
        dxy_pct = dxy_close.pct_change().iloc[-1]
        return prices, float(dxy_pct) if not np.isnan(dxy_pct) else 0.0
    except:
        return None, 0.0

def main():
    engine = XTINeuralEngine()
    st.markdown("### < XTI/USD NEURAL TERMINAL v9.7 >")
    ...
    with tab_ops:
        ...
        prices_raw, dxy_delta = get_market_intelligence()
        
        # Proteção contra IndexError
        if prices_raw is None or len(prices_raw) < 2:
            st.warning("Aguardando uplink de dados do mercado...")
            prices = [0.0, 0.0]
            z_score = 0.0
        else:
            prices = prices_raw
        ...
        with col_verdict:
            ...
            if prices[-1] != 0:
                ...
                # CORREÇÃO AQUI: Substituído use_container_width=True por width='stretch'
